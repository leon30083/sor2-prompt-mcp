...
- 统一分段规则（分段标记优先，其次仅识别 `### 标题`；若无则先转换为该结构）、时长策略（默认 12s，上限 15s）、字段映射（用户示例样式）。
- 输出两类结构：基础 shots JSON 与用户样式 JSON（`shot_type`、`frame_content` 等）。

## 能力概览
 - 文本分段：支持“分段标记”优先；其次仅识别 `### 标题`；若用户输入没有这种结构，必须先处理成这种结构（生成标准化 Markdown，保留 `###`）。分段标记格式：`[SEGMENT: 标题]` 或 `<!-- segment: 标题 -->`。
...
- 分段规则（format_script）：
  ...
  - 若文本既无分段标记也无 `###` 标题，执行预处理：`normalize_to_titles`，自动生成 `### 片段N：标题` 结构，再按标题分段；若文本整体为空也会生成 `### 片段1`。
 - 原始文稿模式（预览与生成约束）：
...
- 步骤：
  - 1) 标准化文本（统一引号、清理空白）
  - 2) 分段并输出 Markdown 预览（仅识别分段标记或 `###`；若无则先转换为 `### 片段N：标题`，保留 `###`）
